...
<body>
    <div class="container py-5">
        ...
        <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT') <!-- Ini untuk menunjukkan bahwa request ini adalah PUT, yang digunakan untuk update -->
            <div class="row">
                <div class="col-lg-8">
                    <!-- Main Content Card -->
                    <div class="card mb-4">
                        <div class="card-body p-4">
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control form-control-lg" id="title" name="title" placeholder="Enter blog title" value="{{ old('title', $blog->title) }}" required>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="content" id="summernote" class="form-control" rows="100">{{ old('content', $blog->content) }}</textarea>
                    ...
                    <!-- Featured Image Card -->
                   @php
    $existingFeatured = $blog->featured_image ?? null;
@endphp
...
<div class="card mb-4">
    <div class="card-body p-4">
        ...
        <div class="image-preview position-relative">
            <div class="image-preview-placeholder text-center {{ $existingFeatured ? 'd-none' : '' }}">
            ...
            <img src="{{ $existingFeatured ? asset('storage/' . $existingFeatured) : '' }}"
                 id="featured-preview-img"
                 class="img-fluid {{ $existingFeatured ? '' : 'd-none' }} mt-3" />
            <div class="image-preview-actions {{ $existingFeatured ? '' : 'd-none' }} mt-2">
        ...
        <input type="hidden" name="remove_featured" id="remove_featured" value="0">
        ...
        @enderror
                    ...
                    <!-- Gallery Images Card -->
                    @php
    $existingGallery = json_decode($blog->gallery_images, true) ?? [];
@endphp
...
<div class="card mb-4">
    <div class="card-body p-4">
        ...
        <div class="gallery-preview d-flex flex-wrap gap-3 mt-3" id="gallery-preview-container">
            @foreach ($existingGallery as $i => $img)
            
                <div class="position-relative" style="width: 150px; height: 150px;">
                    <img src="{{ asset('storage/' . $img) }}" class="img-fluid" style="width: 100%; height: 100%; object-fit: cover; border-radius: 6px;">
                    <button type="button" class="gallery-action-btn position-absolute top-0 end-0 m-1 remove-existing-gallery" data-index="{{ $i }}">
                        <i class="bi bi-trash"></i>
                    </button>
        ...
        <div class="form-text">You can upload multiple images for your blog gallery.</div>
    </div>
</div>
                </div>
                                ...
                                <div class="publish-options">
                                    <div class="publish-option">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status" id="publish-now" value="published" {{ old('status', $blog->status) == 'published' ? 'checked' : '' }}>
                                    ...
                                    <div class="publish-option">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status" id="publish-schedule" value="scheduled" {{ old('status', $blog->status) == 'scheduled' ? 'checked' : '' }}>
                                        ...
                                        <div class="schedule-options d-none" id="schedule-options">
                                            <div class="mb-3">
                                                ...
                                                <input type="datetime-local" class="form-control" id="published_at" name="published_at" value="{{ old('published_at', $blog->published_at) }}">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="publish-option">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status" id="publish-draft" value="draft" {{ old('status', $blog->status) == 'draft' ? 'checked' : '' }}>
    ...
    let index = 0;
      document.querySelectorAll('.remove-existing-gallery').forEach(btn => {
        btn.addEventListener('click', function () {
            const wrapper = this.closest('div.position-relative');
            const index = this.getAttribute('data-index');
            let removedList = JSON.parse(document.getElementById('removed_gallery_images').value);
            removedList.push(index);
            document.getElementById('removed_gallery_images').value = JSON.stringify(removedList);
